...
from info import CHANNELS, ADMINS, USERNAME, AUTH_CHANNEL, LOG_CHANNEL, WRM, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT_ID, SUPPORT_CHAT, MAX_B_TN, VERIFY, SHORTLINK_API, SHORTLINK_URL, TUTORIAL, IS_TUTORIAL, PREMIUM_USER
...
async def start(client, message):
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                ...
                                InlineKeyboardButton('𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥', url=f'https://t.me/Anand_2K')
                            ]
                        ]
                    )
                )
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                         [
                          ...
                          InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url='https://t.me/Anand_2K')
                            ]
                        ]
                    )
                )
        ...
        k = await client.send_message(chat_id=message.from_user.id,text=f"<b>Get All Files in a Single Click!!!\n\n📂 ʟɪɴᴋ ➠ : {g}\n\n<i>Note: This message is deleted in 20 minutes to avoid copyrights. Save the link to Somewhere else</i></b>", reply_markup=InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton('🚀 Fast Download / Watch Online🖥️', callback_data=f'stream_link_generate:{file_id}') #Don't change anything without contacting me @creatorrio
                    ],[
                        InlineKeyboardButton('𝚆𝚎𝚎𝚔𝚕𝚢 𝚁𝚎𝚕𝚎𝚊𝚜𝚎𝚍 𝙼𝚘𝚟𝚒𝚎𝚜', url=WRM)
                    ], [
                        InlineKeyboardButton('💸 Buy Premium For Adz Free Movies ✅', callback_data='buy_premium')
        ...
        for file in files:
            ...
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
                    ...
                    InlineKeyboardButton("Hᴏᴡ Tᴏ Vᴇʀɪғʏ", url='https://t.me/AR_linkz/10')
                ]]
                await message.reply_text(
    ...
    if not files_:
        ...
        try:
            ...
            title = '@AR_linkz  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@AR_linkz  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@AR_linkz  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start=")),
            InlineKeyboardButton("Hᴏᴡ Tᴏ Vᴇʀɪғʏ", url='https://t.me/AR_linkz/10')            
